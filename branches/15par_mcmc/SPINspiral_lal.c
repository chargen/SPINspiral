...
void templateLAL12(struct parSet *par, struct interferometer *ifo[], int ifonr, int injectionWF, struct runPar run)
{
  ...
  double delay = LALFpFc(&status, &waveform, &injParams, &ppnParams, wave, length, par, ifo[ifonr], ifonr, run); //Vivien: lentghLAL is set in LALinteface.c But is is also availble in the structure thewaveform (which holds h+,x) and the structure wave (which holds F+,x)
 ...
 */
// ****************************************************************************************************************************************************  
double LALFpFc(LALStatus *status, CoherentGW *waveform, SimInspiralTable *injParams, PPNParamStruc *ppnParams, double *wave, int length, struct parSet *par, struct interferometer *ifo, int ifonr, struct runPar run) 
{
...
LALDetector *getLIGOSouth(REAL8 AIGOang)
{
	     ...
	     LALDetector *detector;
	            ...
	            /* Location of the AIGO detector vertex is approx */
	            ...
	            detector=malloc(sizeof(LALDetector));
	            memset(detector,0,sizeof(LALDetector));
	            XLALCreateDetector(detector,&LIGOSouthFr,LALDETECTORTYPE_IFODIFF);
	            printf("Created LIGO South detector, location: %lf, %lf, %lf\n",detector->location[0],detector->location[1],detector->location[2]);
	            printf("Detector tensor:\n");
	            for(jdx=0;jdx<3;jdx++){
		                for(j=0;j<3;j++) printf("%f",detector->response[jdx][j]);
		                printf("\n");
	                 }
		    return detector;
}
